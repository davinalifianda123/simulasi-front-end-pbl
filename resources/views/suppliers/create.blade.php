        ...
        <div class="p-4">
            <form action="{{ route('suppliers.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="alamat" id="alamat" rows="3" class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has('alamat') ? 'border-red-500' : 'border-gray-300' }}" required>{{ old('alamat') }}</textarea>
                    @error('alamat')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
