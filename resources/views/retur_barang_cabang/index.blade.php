                    ...
                    @forelse ($cabangKePusats as $cabangKePusat)
                        <tr class="hover:bg-gray-50 cursor-pointer">
                            ...
                            @if(!in_array($nama_role, ['Admin', 'SuperAdmin']))
                                @php
                                    $idVerifikasi = $cabangKePusat->id_verifikasi;
                                ...
                                <td class="flex gap-2">
                                    @if ($statusBadge)
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusBadge }}">
                                            {{ $cabangKePusat->jenis_verifikasi }}
                                        </span>
                                    @else
                                        <!-- Tombol Diverifikasi -->
                                        <form method="POST" action="{{ route('cabang-ke-pusats.update', $cabangKePusat->id) }}">
                                            @csrf
                                            @method('PUT')
        ...
        <div id="bottom-banner" tabindex="-1" class="fixed bottom-0 start-0 z-50 flex justify-between w-full p-4 border-t border-gray-200 bg-gray-50">
            <div class="flex items-center mx-auto">
                <p class="flex items-center text-sm font-normal text-gray-500">
                    ...
                    <span>Gudang sedang di opname, tidak bisa meretur barang untuk saat ini.
