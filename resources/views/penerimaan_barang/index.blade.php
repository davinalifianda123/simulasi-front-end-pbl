    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-center items-center">
            <table id="export-table"
                data-create-route="{{ $status_opname || $nama_role === 'Supervisor' ? '#' : route('penerimaan-di-pusats.create') }}"
                ...
                <tbody>
                    @forelse ($penerimaanDiPusats as $penerimaanDiPusat)
                        <tr class="hover:bg-gray-50 cursor-pointer">
                            <td hidden>{{ $penerimaanDiPusat->id }}</td>
                            ...
                            @if($nama_role !== 'Supervisor')
                                <td class="text-center">
                                    <form method="POST" action="{{ route('penerimaan-di-pusats.update', $penerimaanDiPusat->id) }}">
                                        ...
                                        <button type="submit">
                                            <input type="checkbox"
                                                {{ $penerimaanDiPusat->diterima ? 'checked' : '' }}
                            ...
                            @if(!in_array($nama_role, ['Admin', 'SuperAdmin']))
                                @php
                                    $idVerifikasi = $penerimaanDiPusat->id_verifikasi;
                                ...
                                <td class="flex gap-2">
                                    @if ($statusBadge)
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusBadge }}">
                                            {{ $penerimaanDiPusat->jenis_verifikasi }}
                                        </span>
                                    @else
                                        <!-- Tombol Diverifikasi -->
                                        <form method="POST" action="{{ route('penerimaan-di-pusats.update', $penerimaanDiPusat->id) }}">
                            ...
                            <td colspan="11" class="py-8">
                                <div class="w-full flex flex-col items-center">
                                    ...
                                    @if(!$status_opname && $nama_role !== 'Supervisor')
                                        <a href="{{ route('penerimaan-di-pusats.create') }}"
