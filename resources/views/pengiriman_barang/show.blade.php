...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="flex flex-col gap-4 sm:rounded-lg bg-white p-3 m-6">
            ...
            <div class="border-t border-gray-200">
                <dl class="px-4 py-5">
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $pusatKeCabang->id }}</dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Tanggal Pengiriman</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ \Carbon\Carbon::parse($pusatKeCabang->tanggal)->format('d M Y H:i') }}</dd>
                    </div>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $pusatKeCabang->kode }}</dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Tujuan Pengiriman</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $pusatKeCabang->nama_cabang }}</dd>
                    </div>
                </dl>
            </div>
        </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $pusatKeCabang->nama_barang }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $pusatKeCabang->jumlah_barang }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $pusatKeCabang->berat_satuan_barang . " " . $pusatKeCabang->satuan_berat }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $pusatKeCabang->nama_kurir }}
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div class="flex flex-row justify-between items-center mt-6 px-4 py-5">
            ...
            <a href="{{ route('pusat-ke-cabangs.invoice', $pusatKeCabang->id) }}" target="_blank" class="text-white bg-[#050708] hover:bg-[#050708]/80 font-medium rounded-md text-sm px-4 py-2 text-center flex items-center gap-2 ml-3 w-fit">
