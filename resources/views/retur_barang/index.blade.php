...
<x-default-layout :nama-user="$nama_user" :nama-role="$nama_role">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="flex justify-center items-center">
            <table id="export-table" data-create-route="{{ $status_opname || $nama_role === 'Supervisor' ? '#' : route('pusat-ke-suppliers.create') }}" data-resource-name="Retur Barang" data-route-name="pusat-ke-suppliers" data-editable="false" data-user-role="{{ $nama_role }}" data-show-action="{{ $nama_role !== 'Supervisor' ? 'true' : 'false' }}">
                <thead>
                    <tr>
                        ...
                        <tr class="hover:bg-gray-50 cursor-pointer">
                            ...
                            @if($nama_role !== 'Supervisor')
                                <td>
                                    ...
                                    @endphp
                                    @if($pusatKeSupplier->id_status === 4)
                            ...
                            @if(!in_array($nama_role, ['Admin', 'SuperAdmin']))
                                @php
                                    $idVerifikasi = $pusatKeSupplier->id_verifikasi;
                                ...
                                <td class="flex gap-2">
                                    @if ($statusBadge)
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusBadge }}">
                                            {{ $pusatKeSupplier->jenis_verifikasi }}
                                        </span>
                                    @else
                                        <!-- Tombol Diverifikasi -->
                                        <form method="POST" action="{{ route('pusat-ke-suppliers.update', $pusatKeSupplier->id) }}">
                                            @csrf
                                            @method('PUT')
