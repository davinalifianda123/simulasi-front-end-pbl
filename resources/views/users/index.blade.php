...
<x-default-layout :nama-user="$nama_user" :nama-role="$nama_role">
    <div class="bg-white rounded-lg shadow-md p-6 flex justify-center items-center">
        ...
        <table id="export-table" data-create-route="{{ route('users.create') }}" data-resource-name="User">
            <thead>
                <tr>
            ...
            <tbody>
                @forelse ($users as $user)
                    <tr class="hover:bg-gray-50 cursor-pointer">
                        ...
                        <td>{{ $user->lokasi->nama_gudang_toko }}</td>
                        <td>{{ $user->flag ? 'Aktif' : 'Tidak Aktif' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-8">
                            <div class="w-full flex flex-col items-center">
                                ...
                                <a href="{{ route('users.create') }}" class="bg-[#E3E3E3] hover:bg-[#161A30] text-[#777777] hover:text-white px-4 py-2 rounded-lg transition duration-200">
                                    + Tambah User
                                </a>
                            </div>
                        </td>
